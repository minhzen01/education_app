...
abstract class ChatRemoteDataSource {
  ...
  Future<UserEntityModel> getUserById(String userId);
}
...
class ChatRemoteDataSourceImpl implements ChatRemoteDataSource {
  ...
  Stream<List<GroupModel>> getGroups() {
    try {
      ...
      final groupsStream = _firestore.collection('groups').snapshots().map((snapshot) {
  ...
  Stream<List<MessageModel>> getMessages(String groupId) {
    try {
      DatasourceUtils.authorizeUser(_auth);
      final messagesStream = _firestore.collection('groups').doc(groupId).collection('messages').orderBy('timestamp', descending: true).snapshots().map((snapshot) {
  ...
  @override
  Future<UserEntityModel> getUserById(String userId) async {
    try {
      ...
      return UserEntityModel.fromMap(userDoc.data()!);
    } on FirebaseException catch (e) {
      throw ServerException(
        message: e.message ?? 'Unknown error occurred',
        statusCode: e.code,
  ...
  Future<void> sendMessage(Message message) async {
    try {
      await DatasourceUtils.authorizeUser(_auth);
      final messageRef = _firestore.collection('groups').doc(message.groupId).collection('messages').doc();
      final messageToUpload = (message as MessageModel).copyWith(id: messageRef.id);
