...
class ResourceController extends ChangeNotifier {
  ...
  ResourceEntity? get resource => _resource;

  String get _pathKey => 'material_file_path${_resource!.id}';

  void init(ResourceEntity resource) {
  ...
  Future<File?> downloadAndSaveFile() async {
    ...
    final file = File('${cacheDir.path}/${_resource!.id}.${_resource!.fileExtension}');
      ...
      downloadTask.snapshotEvents.listen((taskSnapshot) async {
        switch (taskSnapshot.state) {
          case TaskState.running:
            _percentage = taskSnapshot.bytesTransferred / taskSnapshot.totalBytes;
            notifyListeners();
