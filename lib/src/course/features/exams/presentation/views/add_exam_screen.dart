...
class _AddExamScreenState extends State<AddExamScreen> {
  ...
  Future<void> pickExamFile() async {
    ...
    if (result != null) {
      setState(() => examFile = File(result.files.single.path!));
    }
  }
  ...
  Future<void> uploadExam() async {
    ...
    if (formKey.currentState!.validate()) {
      ...
      final exam = ExamModel.fromUploadMap(jsonMap).copyWith(courseId: courseNotifier.value!.id);
    ...
    return NotificationWrapper(
      ...
      child: BlocListener<ExamCubit, ExamState>(
        listener: (context, state) {
          ...
          if (state is UploadingExamState) {
            CoreUtils.showLoadingDialog(context);
            showingDialog = true;
          } else if (state is ExamErrorState) {
            CoreUtils.showSnackBar(context, state.message);
          } else if (state is ExamUploadedState) {
            ...
            CoreUtils.sendNotification(
              ...
              body: 'A new exam has been added for ${courseNotifier.value!.title}',
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton(
                          ...
                          child: Text(examFile == null ? 'Select Exam File' : 'Replace Exam File'),
                        ),
